# build_paquetes.ps1 — Compila y empaqueta HADES v1.1 + Simulador para distribución Windows
#
# Genera:
#   installer_output/HADES_v1.1_Windows.zip        (app principal, ~100 MB, JRE embebido)
#   installer_output/HADES_Simulador_v1.1_Windows.zip  (simulador standalone, ~2 MB, requiere Java 17+)
#
# Requisitos:
...
$DIST    = "$ROOT\dist_package\HADES_v1.1"
...
Write-Host ""
Write-Host "[2/6] Preparando staging HADES_v1.1..."
...
@"
╔══════════════════════════════════════════════════════════════╗
║         HADES / HarmonicMonitor v1.1 — Windows              ║
...
Write-Host ""
Write-Host "[5/6] Empaquetando HADES_v1.1_Windows.zip..."

$zipHades = "$OUT_DIR\HADES_v1.1_Windows.zip"
if (Test-Path $zipHades) { Remove-Item $zipHades }
Push-Location "$ROOT\dist_package"
Compress-Archive -Path "HADES_v1.1" -DestinationPath $zipHades -CompressionLevel Optimal
Pop-Location
...
Write-Host ""
Write-Host "[6/6] Empaquetando HADES_Simulador_v1.1_Windows.zip..."

$simStage = "$OUT_DIR\HADES_Simulador_v1.1"
...
$zipSim = "$OUT_DIR\HADES_Simulador_v1.1_Windows.zip"
if (Test-Path $zipSim) { Remove-Item $zipSim }
Push-Location $OUT_DIR
Compress-Archive -Path "HADES_Simulador_v1.1" -DestinationPath $zipSim -CompressionLevel Optimal
Pop-Location
